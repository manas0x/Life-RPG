  ...
  return (
    <div className="rounded-[20px] overflow-hidden border border-white/[0.06] glass">
      ...
      <div className="relative p-5 sm:p-6 bg-gradient-to-br from-obsidian-700 via-obsidian-800 to-obsidian-900">
        <div className="absolute inset-0 opacity-20" style={{backgroundImage:"radial-gradient(circle at 30% 20%, rgba(255,140,66,0.3), transparent 40%)"}} />
        <div className="relative flex items-start justify-between gap-4">
          <div className="flex gap-4">
            <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-zinc-800 to-zinc-900 border border-white/10 flex items-center justify-center text-3xl shadow-xl relative overflow-hidden">
            ...
            <div>
              <h2 className="font-display font-bold text-lg sm:text-xl leading-tight">{username}</h2>
              <p className="text-sm text-zinc-400">Level {level} • Wayfarer</p>
          ...
          <div className="text-right">
            ...
            <p className="text-[11px] font-mono tracking-widest text-zinc-500 mt-1">GOLD</p>
          </div>
        </div>

        {/* XP bar */}
        <div className="relative mt-5">
          <div className="flex justify-between text-[11px] font-mono tracking-widest text-zinc-500 mb-1.5">
            <span>XP PROGRESS</span><span>{xp} / {nextXp}</span>
          </div>
      ...
      <div className="p-4 sm:p-5 grid grid-cols-3 gap-3 border-t border-white/5 bg-obsidian-900/50">
        ...
        <Stat icon={Trophy} label="Level" value={level} sub={`#${String(level).padStart(2,'0')} WAYFARER`} />
        <Stat icon={Coins} label="Gold" value={gold} sub="Treasury" />
      </div>

      {/* Attributes */}
      <div className="p-4 sm:p-5 border-t border-white/5">
        <h3 className="text-xs font-mono tracking-[0.18em] text-zinc-500 mb-3">ATTRIBUTES</h3>
        ...
        <p className="text-[11px] text-zinc-500 mt-3 font-mono">Complete quests with matching sigils to grow attributes.</p>
      </div>
    </div>
  );
}
...
function Stat({ icon: Icon, label, value, sub, highlight }) {
  return (
    <div className={`rounded-2xl p-3 border ${highlight ? 'bg-ember-500/10 border-ember-500/20' : 'bg-white/[0.03] border-white/5'} text-center`}>
      ...
      <div className="text-[11px] font-mono tracking-widest text-zinc-500">{label.toUpperCase()}</div>
